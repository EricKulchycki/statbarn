...
import { Division, Team, TeamSeasonGame } from '@/types/team'
...
export interface TeamSeasonGameSerialized
  extends Omit<TeamSeasonGame, 'gameDate'> {
  gameDate: string
}

interface AllTeamsHistoryGraphProps {
  teams: Team[]
  historyByTeamSerialized: { [abbrev: string]: TeamSeasonGameSerialized[] }
}
...
export const AllTeamsHistoryGraph: React.FC<AllTeamsHistoryGraphProps> = ({
  ...
  const historyByTeam: { [abbrev: string]: TeamSeasonGame[] } = {}
  for (const [abbrev, games] of Object.entries(historyByTeamSerialized)) {
    historyByTeam[abbrev] = games.map((g) => ({
      ...g,
      gameDate: new Date(g.gameDate),
    }))
  }

  delete historyByTeam['ARI']

  const filteredTeams = teams.filter((t) => t.division === selectedDivision)
  const teamAbbrevs = Array.from(new Set(filteredTeams.map((t) => t.triCode)))

  const allDatesSet = new Set<string>()
  teamAbbrevs.forEach((abbrev) => {
    ;(historyByTeam[abbrev] || []).forEach((g) => {
      allDatesSet.add(new Date(g.gameDate).toLocaleDateString())
    })
  })
  ...
  const chartData = allDates.map((date) => {
    ...
    for (const abbrev of teamAbbrevs) {
      ...
      if (game) {
        const elo = game.outcome?.eloAfter ?? game.eloBefore
        entry[abbrev] = elo
        ...
        {payload.map((entry) => (
          ...
          >
            <Image
              width={24}
              height={24}
              src={teams.find((t) => t.triCode === entry.value)?.logo ?? ''}
              className="mr-1"
              alt={entry.value}
            />
            {entry.value}
